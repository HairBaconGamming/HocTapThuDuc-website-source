/**
 * GARDEN PHASER ENGINE - ASSETS FIXED 1:1
 * Update: Fix size cỏ 1:1, dùng assets mới cho đất/nước.
 */
...
function showToast(m, i = 'info') {
    if (window.SwalPixel) window.SwalPixel.fire({ title: m, icon: i });
    else console.log(m);
}

// --- 1. DỮ LIỆU HỘI THOẠI NPC ---
const NPC_DIALOGUES = {
    welcome: ["Hế lô cưng! Nay mua gì nè? 💖", "Vào lựa lẹ đi, hàng mới về nóng hổi! 🔥", "Trộm vía, nay nhìn bạn tươi nha!", "Khách quen, giảm giá... trong mơ nha! 🤣"],
    plants: ["Trồng cây đi, đừng trồng cây si! 🌱", "Hạt giống bao nảy mầm!", "Mua Lúa đi, sau này tha hồ gặt hái! 🌾", "Hướng dương nè, nhìn là thấy chói chang! 🌻"],
    decors: ["Decor cái vườn cho nó keo lỳ đi fen! ✨", "Nhà sạch thì mát, vườn đẹp thì tốn tiền! 💸", "Mấy cái này đang trend đó!", "Vườn đẹp là auto có người yêu nha!"],
    backgrounds: ["Đất chật người đông, mở rộng ra cho thoáng! 🚜", "Muốn làm đại gia đất đai thì bơi vào đây!", "Mua thêm đất đi, trồng cho đã cái nư!"]
};
...
class MainScene extends Phaser.Scene {
    ...
    preload() {
        ...
        // [LOAD] Đất & Cỏ từ Assets Config
        if (ASSETS.PLOT && ASSETS.PLOT.grass) {
            this.load.image('grass_tile', ASSETS.PLOT.grass.textureUrl);
        }
        if (ASSETS.FARMING) {
        ...
        for (let key in ASSETS.DECORS) {
            ...
            if (item.isFence) {
                // Load các biến thể của rào (Nếu bạn có full bộ 16 hình thì load kiểu khác)
                // Ở đây mình demo dùng 3 hình cơ bản ghép lại
                // Bạn hãy thay thế bằng link ảnh thật
                this.load.image(`decor_${key}_base`, 'https://i.imgur.com/8BiKzXw.png'); // Ảnh cọc
                this.load.image(`decor_${key}_h`, 'https://i.imgur.com/GzJzM9S.png');    // Ảnh ngang
                this.load.image(`decor_${key}_v`, 'https://i.imgur.com/vPqAlqL.png');    // Ảnh dọc
            } else {
    ...
    create() {
        ...
        // [FIX 1:1] Background Grass Tile
        // Tạo background
        this.bgTile = this.add.tileSprite(0, 0, WORLD_W, WORLD_H, 'grass_tile').setOrigin(0);
        
        // [QUAN TRỌNG] Tự động scale ảnh cỏ để vừa khít grid 64x64
        if (this.textures.exists('grass_tile')) {
            ...
            if (frame) {
                ...
                this.bgTile.setTileScale(scaleX, scaleY);
            }
        }
        ...
        this.selectionMarker = this.add.graphics().lineStyle(4, 0x00ffff, 1).strokeRect(0, 0, GRID_SIZE, GRID_SIZE).setDepth(99998).setVisible(false);
        ...
        this.input.on('pointerup', () => {
            if (this.isPanning) {
                ...
                if (this.currentTool === 'move') this.input.setDefaultCursor('grab');
        ...
        this.input.on('pointermove', (pointer) => {
            ...
            if (this.movingSprite) {
                ...
                if (this.movingSprite.thirstyIcon) {
                    ...
                    if (plotUnderneath && plotUnderneath.itemData.lastWatered) {
                        const now = Date.now();
                        if (now - new Date(plotUnderneath.itemData.lastWatered).getTime() < 24 * 60 * 60 * 1000) isGroundWet = true;
                    }
                ...
                let color = 0xffffff;
                if (this.currentTool === 'move') color = 0xaaaaaa;
            ...
            if (this.selectedTile) {
                ...
                if (this.selectedTile.itemData.type !== 'plot') {
                     drawX -= w/2; drawY -= h;
                }
        ...
        this.input.keyboard.on('keydown-X', () => {
            if (window.cancelPlantingMode) window.cancelPlantingMode();
            else {
                this.selectedTile = null;
                window.hidePlantStats();
        ...
        this.input.keyboard.on('keydown-R', () => {
            if (this.selectedTile && this.currentTool === 'cursor') this.startMovingSprite(this.selectedTile);
        });
        ...
        window.gameEvents.on('toolChanged', (t) => {
            this.currentTool = t;
            if (t === 'move') this.input.setDefaultCursor('grab');
            else this.input.setDefaultCursor('default');
            this.updateCursor(t);
            this.isPanning = false;
            if (this.movingSprite) this.cancelMove();
            this.selectedTile = null;
            window.hidePlantStats();
        });
        ...
        if (this.currentTool === 'hoe') {
            if (!plot) {
                ...
                if (res.success) {
                    ...
                    if (res.msg.includes('vàng')) this.showFloatingText(gx + 32, gy, "Thiếu Tiền! 💸", 'red');
                }
            }
        }
        // --- TOOL: WATER ---
        else if (this.currentTool === 'water') {
            if (plant && plant.itemData.isDead) {
                showToast('Cây hẹo rồi! 🥀', 'error');
                this.showFloatingText(plant.x, plant.y - 40, "Đã Chết 💀", 'gray');
                return;
            }
            if (plant || plot) {
                ...
                if (res.success) {
                    ...
                    if (res.msg.includes('nước')) this.showFloatingText(gx + 32, gy, "Hết Nước! 😩", "red");
                }
            }
        }
        // --- TOOL: SHOVEL ---
        else if (this.currentTool === 'shovel') {
            if (plant) this.removeObj(plant);
            else if (plot) this.removeObj(plot);
        }
        // --- TOOL: BASKET ---
        else if (this.currentTool === 'basket') {
            if (plant && plant.itemData.type === 'plant') {
                if (plant.itemData.isDead) {
                    showToast('Cây hẹo rồi! Dùng xẻng dọn đi!', 'error');
                    this.showFloatingText(plant.x, plant.y - 40, "Đã Chết 💀", 'gray');
                    return;
                }
                ...
                if (res.success) {
                    ...
                    this.tweens.add({
                        ...
                        onComplete: () => {
                            ...
                            if (plant.miniHud) plant.miniHud.destroy();
    ...
    renderItem(item) {
        ...
        // [LOGIC MỚI CHO FENCE]
        if (config.isFence) {
            // Mặc định load ảnh cọc trước, lát nữa hàm updateFenceVisuals sẽ chỉnh lại sau
            key = `decor_${item.itemId}_base`; 
        } else if (item.type === 'plant') {
            key = `plant_${item.itemId}_${item.stage || 0}`;
        } else {
            key = `decor_${item.itemId}`;
        }
        ...
        const sprite = this.add.sprite(
            ...
            item.y + (h * GRID_SIZE), // Y tại đáy của ô
            key
        ).setOrigin(0.5, 1); // Gốc tại giữa đáy
        ...
        // Sau khi render xong, nếu là Fence thì cập nhật hình ảnh kết nối
        if (config.isFence) {
            // Gọi delay nhẹ để đảm bảo tất cả item đã load xong
            this.time.delayedCall(50, () => this.updateAllFences());
        }
        return sprite;
    }

    // Cập nhật lại hình ảnh cho TẤT CẢ hàng rào (gọi khi đặt mới hoặc xóa)
    updateAllFences() {
    ...
    updateFenceTexture(sprite) {
        ...
        const x = item.x;
        const y = item.y;
        const id = item.itemId; // Ví dụ 'fence_wood'

        // Kiểm tra 4 hướng: Bắc, Tây, Đông, Nam
        // Grid size = 64
        ...
        // Logic chọn ảnh đơn giản (Có thể mở rộng thành 16 trường hợp)
        let textureKey = `decor_${id}_base`; // Cọc đơn

        if (hasLeft || hasRight) {
            if (!hasTop && !hasBottom) {
                textureKey = `decor_${id}_h`; // Chỉ nối ngang
            } else {
                // Nếu vừa có ngang vừa có dọc -> Có thể dùng ảnh 'Cross' hoặc giữ nguyên Base
                // Ở đây mình ưu tiên Base (Cọc) nếu nối 3-4 hướng để nhìn giống cột trụ
                textureKey = `decor_${id}_base`; 
            }
        } 
        
        if (hasTop || hasBottom) {
            if (!hasLeft && !hasRight) {
                textureKey = `decor_${id}_v`; // Chỉ nối dọc
            }
        }

        // [TINH CHỈNH NÂNG CAO]
        // Nếu bạn muốn nối liền mạch thực sự, bạn cần bộ tileset 16 hình (bitmask 0-15)
        // Đây là cách đổi texture:
        sprite.setTexture(textureKey);
        
        // Reset lại size vì setTexture có thể làm mất size custom
        const w = ASSETS.DECORS[id].size.w;
        const h = ASSETS.DECORS[id].size.h;
        sprite.setDisplaySize(w * GRID_SIZE, h * GRID_SIZE);
    }

    hasFenceAt(x, y, itemId) {
        return this.children.list.some(o => 
            o.isGardenItem && 
            o.itemData.x === x && 
            o.itemData.y === y && 
            o.itemData.itemId === itemId
        );
    }
    ...
    startMovingSprite(sprite) {
        ...
        if (canMove) {
            ...
            showToast(item.isDead ? 'Cây chết không dời được!' : 'Cây đang lớn, đừng động!', 'warning');
        }
    }
    ...
    placeMovingSprite() {
        ...
        const config = ASSETS.DECORS[this.movingSprite.itemData.itemId];
        if (config && config.isFence) {
    ...
    async removeObj(obj) {
        ...
        if (res.success) {
            ...
            const config = ASSETS.DECORS[obj.itemData.itemId];
            if (config && config.isFence) {
                // Đợi object bị destroy xong mới update
    ...
    update() {
        this.children.list.forEach(child => {
            if (child.isGardenItem) {
                if (child === this.movingSprite) {
                    child.setDepth(999999); // Đang kéo thì luôn nổi
                } else {
    ...
    showFloatingText(x, y, message, color) {
        ...
        const txt = this.add.text(x, y - 20, message, {
            fontFamily: 'VT323', fontSize: '32px', color: hexColor,
            stroke: '#000', strokeThickness: 4, align: 'center'
        }).setOrigin(0.5).setDepth(999999);

        this.tweens.add({
            targets: txt, y: y - 100, alpha: 0, duration: 2000, ease: 'Power2',
    ...
    showLevelUpEffect(sprite) {
        ...
        this.add.particles(0, 0, 'star_particle', {
            ...
            speed: { min: 50, max: 150 }, scale: { start: 0.4, end: 0 }, lifespan: 800, quantity: 5
        }).explode();
    }
...
async function apiCall(u, b) {
    try {
        return await (await fetch(u, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(b) })).json();
    } catch (e) { return { success: false }; }
}

function updateHUD(d) {
    if (d.newWater !== undefined) document.getElementById('hudWater').innerText = d.newWater;
    if (d.newGold !== undefined) document.getElementById('hudGold').innerText = d.newGold;
}

window.buyItemPhaser = async function (i, t) {
    document.getElementById('shopOverlay').style.display = 'none';
    showToast('Chọn ô đất để trồng 🌱', 'info');

    const s = game.scene.keys['MainScene'];

    const cancelBtn = document.createElement('button');
    cancelBtn.innerHTML = '❌ Hủy bỏ (X)';
    cancelBtn.style.cssText = 'position:fixed; bottom:100px; left:50%; transform:translateX(-50%); padding:10px 20px; background:#ef5350; color:white; border:none; border-radius:20px; font-family:"VT323"; font-size:1.5rem; cursor:pointer; box-shadow: 0 4px 0 #b71c1c; z-index:10000;';
    ...
    const plantHandler = async (p) => {
        ...
        if (r.success) {
            ...
            if (r.msg.includes('vàng')) s.showFloatingText(x + 32, y, "Thiếu Tiền! 💸", 'red');
        }
    ...
    window.cancelPlantingMode = function () {
        ...
        showToast('Đã hủy trồng cây', 'info');
    };
};
...
window.populateShop = function (category) {
    ...
    if (category === 'backgrounds') {
        grid.innerHTML = `<div style="grid-column: 1/-1; text-align: center; color: #5d4037; padding: 20px;"><div style="font-size: 3rem; margin-bottom: 10px;">🚜</div><div>Dùng <b>CÂY CUỐC</b> để mở rộng đất nhé!</div></div>`;
        return;
    }
    ...
    Object.keys(items).forEach(key => {
        ...
        if (!isLocked) {
            div.onclick = () => window.buyItemPhaser(key, item.type);
        } else {
            div.onclick = () => {
                showToast(`Cần cấp ${requiredLevel} để mở khóa!`, 'error');
                div.animate([{ transform: 'translateX(0)' }, { transform: 'translateX(-5px)' }, { transform: 'translateX(5px)' }, { transform: 'translateX(0)' }], { duration: 300 });
            };
        }
...
window.openShopHTML = function (tab = 'plants') {
    ...
    if (win) {
        ...
        win.style.opacity = '0';
        setTimeout(() => {
            win.style.transition = 'all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275)';
